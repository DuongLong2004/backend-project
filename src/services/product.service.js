 ...
 */
const sanitizeFulltextSearch = (str) => str.replace(/[+\-><()~*"@'\\]+/g, " ").trim();
...
exports.getProducts = async (query) => {
  ...
  if (search && search.trim()) {
    ...
    if (ftQuery) {
      ...
      extraWhere.push(
        sequelize.literal(`MATCH(title, brand, description) AGAINST(${escaped} IN BOOLEAN MODE)`)
      );
    }
  }
