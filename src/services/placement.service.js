...
exports.getPlacements = async (placement) => {
  ...
  if (placement === "flashsale") {
    where[Op.or] = [{ saleEndAt: null }, { saleEndAt: { [Op.gte]: now } }];
  }
...
exports.createPlacement = async ({
  ...
  return ProductPlacement.create({
    ...
    stockLimit: placement === "flashsale" && stockLimit != null ? parseInt(stockLimit, 10) : null,
    stockSold: 0,
  });
};
...
exports.reorderPlacements = async ({ placement, items }) => {
   ...
   */
  const cases = items
    .map(({ id, sortOrder }) => `WHEN ${sequelize.escape(id)} THEN ${sequelize.escape(sortOrder)}`)
    .join(" ");
